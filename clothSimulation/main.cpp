...
const vec3 cameraPosition = vec3(-2.0f, 0.0f, 0.0f); // angle from x, better view no mouse click, frustum needs to be changed to
...
void drawTriangles(vector<glm::vec3> particles, Shader phongShader, GLuint RenderShader) {
	...
	// Frustum for x angle view
	glm::mat4 frustum = glm::frustum(0, 2, -2, 0, clipingPlaneNear, 1); // left, right, bottom, top, near, far
